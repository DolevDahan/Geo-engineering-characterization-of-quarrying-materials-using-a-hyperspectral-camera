...
% 
% This function takes a hyperspectral data cube as input (3D double array) and normalizes each
...
%
% Input:
%   DataCubeMatrix - A hyperspectral data cube (3D double array), provided as a 3D matrix of size
%                    (rows x cols x bands), where each pixel contains spectral
%                    information across multiple bands (wavelengths).
%
% Output:
%   NormalizedMatrix - A hyperspectral data cube (3D double array) where each pixel vector has
%                      been normalized to unit length (i.e., the Euclidean distance 
%                      of each vector is 1).
%   EuclideanDistanceMatrix - A 2D matrix (double array) of size (rows,cols) containing the
%                             original Euclidean distances of each pixel
%                             vector.
%
% Example:
%   % Assuming DataCube_Rock45d is a (3D double array) matrix of hyperspectral data (rows x cols x bands)
